...
import { pickerSelectStyles, styles } from "@/components/Calculator/styles";
...
const OldCalculator = () => {
  ...
  const saveNewType = () => {
    ...
    if (typeTarget === "holiday")
      setHolidayTypes((prev) => [...prev, typeItem]);
    if (typeTarget === "premium")
      setPremiumTypes((prev) => [...prev, typeItem]);
    if (typeTarget === "overtime")
      setOvertimeTypes((prev) => [...prev, typeItem]);
  ...
  const calculate = () => {
    const dr =
      parseFloat(inputs.actualRate) || parseFloat(inputs.prevailingRate) || 0;
  ...
  const renderPickerWithAdd = (label, items, targetField, addTarget) => (
    <>
      ...
      <TouchableOpacity
        ...
        onPress={() => openAddTypeModal(addTarget)}
      >
      ...
      >
        {calculators.map((item, index) => (
          <TouchableOpacity
            ...
            style={[
              styles.calcButton,
              selectedCalc === item.label && styles.calcButtonActive,
            ]}
            onPress={() => {
          ...
          >
            ...
            <Text
              style={[
                ...
                selectedCalc === item.label && styles.calcButtonTextActive,
              ]}
            >
              {item.label}
            </Text>
